...
Nghiên cứu dự đoán stress từ dữ liệu cảm biến có thể chia thành ba hướng chính: (i) hướng truyền thống dựa trên luật/ngưỡng sinh lý và mô hình thống kê; (ii) hướng học máy cổ điển (SVM, Random Forest, LDA); (iii) hướng học sâu khai thác dữ liệu chuỗi đa cảm biến \cite{garciaceja2018stress, dalmeida2021stress, rashid2024stress}. Để tránh cách trình bày liệt kê thuần túy, phần này sử dụng khung so sánh thống nhất nhằm chỉ ra rõ ràng điểm mạnh, điểm yếu và khoảng trống nghiên cứu.

\subsection{Tiêu chí so sánh công trình liên quan}

Các nghiên cứu được đối chiếu theo năm tiêu chí:
\begin{enumerate}
    \item \textbf{Loại mô hình:} truyền thống, học máy cổ điển hay học sâu chuỗi thời gian.
    \item \textbf{Mức độ tích hợp ngữ cảnh hoạt động:} có hoặc không mô hình hóa hoạt động (HAR/context).
    \item \textbf{Dạng đầu ra:} phân loại rời rạc hay hồi quy liên tục.
    \item \textbf{Giá trị thực nghiệm:} mức hiệu năng báo cáo trong bối cảnh dữ liệu tương ứng.
    \item \textbf{Hạn chế cốt lõi:} điểm nghẽn còn tồn tại, làm cơ sở xác định research gap.
\end{enumerate}

\begin{table}[H]
\centering
\caption{So sánh nghiên cứu liên quan theo tiêu chí phương pháp và khoảng trống}
\label{tab:related_work}
\scriptsize
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|p{2.3cm}|p{2.2cm}|p{1.8cm}|p{1.8cm}|p{2.7cm}|p{2.8cm}|}
\hline
\textbf{Nghiên cứu} & \textbf{Kỹ thuật chính} & \textbf{Ngữ cảnh hoạt động} & \textbf{Dạng đầu ra} & \textbf{Giá trị thực nghiệm} & \textbf{Hạn chế cốt lõi} \\
\hline
Kusserow et al. \cite{kusserow2013monitoring} & Luật/ngưỡng sinh lý trong môi trường thực & Không tường minh & Nhị phân & Chứng minh tính khả thi giám sát stress ngoài phòng thí nghiệm & Dễ nhiễu theo cá nhân; khó mở rộng cho mô hình dự đoán liên tục \\
\hline
Hovsepian et al. \cite{hovsepian2015cstress} & Đa đặc trưng sinh lý + SVM/regression pipeline & Không tường minh & Nhị phân & F1 = 0.72 trong bối cảnh mobile sensing & Chưa tích hợp HAR; mất thông tin mức độ stress giữa các ngưỡng \\
\hline
Schmidt et al. \cite{schmidt2018wesad} & Random Forest, LDA, AdaBoost trên WESAD & Gián tiếp (gia tốc kế) & 3 lớp & Acc xấp xỉ 80\% & Chủ yếu bối cảnh phòng thí nghiệm; chưa có cơ chế diễn giải ngữ cảnh hoạt động trong suy luận stress \\
\hline
Can et al. \cite{can2019stress} & CNN/LSTM trên tín hiệu đeo tay & Không tường minh & Nhị phân & Acc = 83.5\% trong kịch bản thực địa có kiểm soát & Chưa xử lý rõ nhập nhằng sinh lý do khác biệt hoạt động; đầu ra vẫn rời rạc \\
\hline
\textbf{Khóa luận này} & \textbf{HAR + Context-Stress Modifiers + Stacked Bi-LSTM + Bayesian Optimization} & \textbf{Tường minh (HAR + context-aware)} & \textbf{Hồi quy liên tục (1--9)} & $\boldsymbol{R^2 = 0.9555}$, MAE thấp trên tập kiểm tra & Cần kiểm chứng thêm trên bộ dữ liệu stress thu thập hoàn toàn thực địa quy mô lớn \\
\hline
\end{tabular}
\end{table}

Từ so sánh trên, khóa luận xác định bốn khoảng trống nghiên cứu trọng tâm:

\textbf{Khoảng trống G1 — Bài toán thường bị rời rạc hóa:} Phần lớn nghiên cứu vẫn dự đoán stress theo nhãn rời rạc (nhị phân/đa lớp), trong khi stress biến thiên liên tục theo thời gian. Điều này làm giảm khả năng theo dõi biến động vi mô của trạng thái tâm lý.

\textbf{Khoảng trống G2 — Thiếu mô hình hóa ngữ cảnh hoạt động tường minh:} Nhiều hệ thống dùng tín hiệu sinh lý trực tiếp mà không gắn với hoạt động đang diễn ra, dẫn tới nhập nhằng sinh lý. Khoảng trống này được kiểm chứng qua \textbf{RQ1/H1} trong khóa luận.

\textbf{Khoảng trống G3 — Thiếu benchmark kiến trúc chuỗi thời gian có kiểm soát:} Một số công trình dùng LSTM/CNN nhưng thiếu so sánh hệ thống giữa mô hình chuỗi và phi chuỗi trong cùng pipeline dữ liệu. Khoảng trống này được xử lý qua \textbf{RQ2/H2}.

\textbf{Khoảng trống G4 — Thiếu quy trình tối ưu và diễn giải nhất quán:} Nhiều nghiên cứu báo cáo hiệu năng nhưng chưa kết nối đầy đủ giữa tối ưu siêu tham số, phân tích đóng góp đặc trưng và tính tin cậy của kết luận. Khoảng trống này được giải quyết thông qua \textbf{RQ3/H3} và \textbf{RQ4/H4}.

Như vậy, đóng góp của khóa luận không chỉ nằm ở một điểm số hiệu năng cao, mà ở việc xây dựng chuỗi lập luận nghiên cứu nhất quán: \textit{research gap} $\rightarrow$ giả thuyết $\rightarrow$ thiết kế thực nghiệm $\rightarrow$ bằng chứng định lượng. Chương tiếp theo trình bày chi tiết cách thiết kế dữ liệu, mô hình và quy trình đánh giá để kiểm chứng các giả thuyết này.
