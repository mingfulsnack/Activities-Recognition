...
\section{Nền tảng lý thuyết cho các hướng tiếp cận truyền thống}
\label{sec:traditional_foundations}

Trước khi đi vào các mô hình học sâu sẽ dùng trong khóa luận, cần làm rõ nền tảng lý thuyết của các hướng tiếp cận cũ trong dự đoán stress. Về bản chất, stress là hiện tượng tâm sinh lý đa yếu tố: cùng một phản ứng sinh lý có thể mang ý nghĩa khác nhau tùy theo hoạt động, thời điểm và bối cảnh môi trường. Các lý thuyết stress kinh điển nhấn mạnh tính động của quá trình này, trong đó trạng thái hiện tại chịu ảnh hưởng đồng thời bởi tác nhân tức thời và tải tích lũy theo thời gian \cite{chrousos2009stress, mcewen2008central}.

Từ góc nhìn mô hình hóa, các kỹ thuật truyền thống thường xuất phát từ hai hướng. Hướng thứ nhất là \textit{rule-based/threshold-based}: đặt ngưỡng cho nhịp tim, HRV hoặc các chỉ số hành vi để cảnh báo stress. Hướng thứ hai là \textit{feature-based machine learning}: trích xuất đặc trưng thủ công (thống kê thời gian/tần số, đặc trưng ngữ cảnh đơn giản) rồi đưa vào các bộ phân lớp hoặc hồi quy cổ điển như Logistic Regression, SVM, Random Forest. Các phương pháp này có ưu điểm là dễ triển khai, dễ diễn giải ở mức cục bộ, và không yêu cầu hạ tầng tính toán lớn.

Tuy nhiên, các tiếp cận cũ cũng có ba hạn chế lý thuyết đáng chú ý. Thứ nhất, chúng phụ thuộc mạnh vào chất lượng đặc trưng thủ công nên khó ổn định khi thay đổi thiết bị hoặc người dùng. Thứ hai, khả năng nắm bắt phụ thuộc thời gian dài còn hạn chế, đặc biệt khi stress biến thiên theo chuỗi hành vi liên tiếp. Thứ ba, nhiều hệ thống vẫn diễn giải stress theo nhãn rời rạc, trong khi nhu cầu ứng dụng thường cần theo dõi cường độ liên tục.

Các hạn chế trên là lý do chương này sẽ lần lượt trình bày nền tảng của nhóm mô hình chuỗi thời gian (RNN/LSTM/Bi-LSTM/GRU), các kỹ thuật huấn luyện-tối ưu, và phương pháp phân tích đặc trưng để khắc phục điểm yếu của hướng truyền thống.
...
Các phương pháp truyền thống cho HAR thường dựa trên trích xuất đặc trưng thủ công (thống kê miền thời gian/tần số) kết hợp với SVM, Decision Tree, hoặc Random Forest \cite{bulling2014tutorial}. Dù hiệu quả trong các kịch bản có kiểm soát, hướng tiếp cận này có ba hạn chế phổ biến: (i) phụ thuộc mạnh vào kinh nghiệm chuyên gia khi thiết kế đặc trưng; (ii) nhạy với lựa chọn cửa sổ thời gian và tần số lấy mẫu; (iii) khó tổng quát hóa tốt khi thay đổi thiết bị, người dùng hoặc bối cảnh thu thập.

Các kiến trúc Deep Learning khắc phục đáng kể các hạn chế trên nhờ khả năng học biểu diễn trực tiếp từ dữ liệu cảm biến thô \cite{ordonez2016deep, chen2021deep}. CNN phù hợp để học mẫu cục bộ (local patterns), trong khi RNN/LSTM nắm bắt phụ thuộc thời gian. Bi-LSTM mở rộng thêm ngữ cảnh hai chiều, giúp nhận dạng ổn định hơn tại các pha chuyển tiếp hoạt động (ví dụ giữa ``đứng'' và ``đi bộ''). Trong phạm vi khóa luận, module HAR sử dụng Stacked Bi-LSTM để tối đa hóa chất lượng nhãn ngữ cảnh trước khi đưa vào mô hình dự đoán stress.
...
\textbf{Tổng hợp hạng (Rank Aggregation).} Mỗi phương pháp có ưu nhược điểm riêng, do đó khóa luận sử dụng cách tiếp cận tổng hợp: xếp hạng các đặc trưng theo từng phương pháp, sau đó tính hạng trung bình để đưa ra đánh giá tổng thể. Phương pháp này giúp giảm thiên lệch từ bất kỳ phương pháp đơn lẻ nào.

% ============================================================

\section{Nghiên cứu liên quan cho bài toán dự đoán stress}
\label{sec:related_work}

Dự đoán stress là một chủ đề quan trọng trong phân tích hành vi và sức khỏe số, với mục tiêu suy luận trạng thái căng thẳng từ dữ liệu cảm biến và ngữ cảnh sinh hoạt hằng ngày. Bài toán này tương tự nhiều nhiệm vụ khai thác tín hiệu thời gian: thông tin đầu vào vừa có thành phần sinh lý, vừa có thành phần hành vi-ngữ cảnh, và mối quan hệ giữa các biến thường không tuyến tính. Vì vậy, tiến trình nghiên cứu trong lĩnh vực thường đi từ các mô hình đơn giản dựa trên luật đến các mô hình học sâu khai thác chuỗi thời gian.

Các công trình giai đoạn đầu chủ yếu tập trung vào đặc trưng sinh lý và thuật toán học máy cổ điển. Kusserow et al. \cite{kusserow2013monitoring} cho thấy tính khả thi của giám sát stress ngoài phòng thí nghiệm bằng cảm biến đeo, nhưng vẫn phụ thuộc mạnh vào thiết kế chỉ báo và ngưỡng. Hovsepian et al. \cite{hovsepian2015cstress} đề xuất pipeline cStress dựa trên đa đặc trưng sinh lý và báo cáo F1 = 0.72 trong kịch bản mobile sensing. Tiếp theo, Schmidt et al. \cite{schmidt2018wesad} so sánh các bộ phân lớp như Random Forest, LDA, AdaBoost trên WESAD và đạt mức chính xác xấp xỉ 80\%. Các kết quả này xác nhận tiềm năng của học máy cổ điển, nhưng đồng thời bộc lộ giới hạn khi phải tự thiết kế đặc trưng thủ công và khó nắm bắt phụ thuộc dài hạn.

Khi học sâu phát triển, các nghiên cứu chuyển sang mô hình hóa chuỗi cảm biến theo hướng tự học biểu diễn. Can et al. \cite{can2019stress} áp dụng CNN/LSTM trên tín hiệu đeo tay và cho thấy cải thiện hiệu suất so với hướng truyền thống trong bối cảnh thực địa có kiểm soát. Các tổng quan gần đây \cite{garciaceja2018stress, dalmeida2021stress, rashid2024stress} cũng ghi nhận xu hướng dùng mô hình chuỗi thời gian đa cảm biến nhằm học quan hệ phức tạp giữa sinh lý, hành vi và thời điểm. Tuy nhiên, phần lớn nghiên cứu vẫn thiên về đầu ra rời rạc (binary/multi-class), trong khi stress thực tế biến thiên liên tục theo cường độ.

Một hạn chế quan trọng khác là mức tích hợp ngữ cảnh hoạt động còn chưa tường minh. Nhiều hệ thống sử dụng trực tiếp tín hiệu sinh lý mà chưa mô hình hóa rõ ràng hoạt động đang diễn ra, dẫn đến nhập nhằng khi diễn giải (ví dụ nhịp tim cao do vận động và nhịp tim cao do căng thẳng tâm lý). Bên cạnh đó, không phải nghiên cứu nào cũng thực hiện benchmark có kiểm soát giữa mô hình chuỗi và phi chuỗi trên cùng pipeline dữ liệu, hoặc kết nối nhất quán giữa tối ưu siêu tham số và phân tích khả năng giải thích.

Từ tổng quan trên, khóa luận định vị khoảng trống nghiên cứu theo bốn điểm: (i) cần chuyển từ dự đoán rời rạc sang hồi quy stress liên tục; (ii) cần tích hợp ngữ cảnh hoạt động theo cách tường minh; (iii) cần so sánh kiến trúc có kiểm soát trong cùng pipeline; và (iv) cần kết hợp tối ưu siêu tham số với phân tích đặc trưng để tăng độ tin cậy của kết luận. Các nội dung này là cơ sở để thiết kế phương pháp đề xuất ở Chương 3 và kiểm chứng thực nghiệm ở Chương 4.
